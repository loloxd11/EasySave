...
namespace EasySave
{
    public class StateManager : IObserver
    {
        ...
        private Dictionary<string, JobStateInfo> stateData= new();
        ...
        public void OnNext(BackupJob job)
        {
            UpdateJobState(job);
        }
        ...
        private void LoadStateFile()
        {
            try
            {
                ...
                if (File.Exists(stateFilePath))
                {
                    ...
                    stateData = JsonSerializer.Deserialize<Dictionary<string, JobStateInfo>>(json) ?? new Dictionary<string, JobStateInfo>();
                }
                else
                {
        ...
        {
            public string Name { get; set; } = string.Empty; // The name of the job
            public string State { get; set; } = string.Empty;// The current state of the job
            ...
            public string CurrentSourceFile { get; set; } = string.Empty; // The current source file being processed
            public string CurrentTargetFile { get; set; } = string.Empty; // The current target file being processed
        }
    }
}
